 ...
 <div class="col-md-9 content-menu">
	...
	if(!empty($_GET)){
		...
		if($_GET['act'] == 'bayar' && $_GET['id']){
			...
			$qPembayaran = mysqli_query($db, "Select * from pembayaran where id_pesanan='$id' and status='verified'") or die (mysqli_error());
			$totalPembayaran = 0;
 		...
 		<div class="row col-md-6">
 			<form action="" method="post" enctype="multipart/form-data">
 				...
 				<input type="text" class="form-control" name="total"
 					value="<?php echo 'Rp. ' . number_format($total+$pesanan->ongkir, 2,',','.'); ?>" disabled
 					required><br>
 				<label>Dibayar</label><br>
 				<input type="text" class="form-control" name="dibayar"
 					value="<?php echo "Rp. ". number_format($totalPembayaran, 2, ",", "."); ?>" disabled required><br>
 				<label>Kekurangan</label><br>
 				<input type="text" class="form-control" name="kekurangan"
 					value="<?php echo "Rp. ". number_format($total+$pesanan->ongkir-$totalPembayaran, 2, ",", "."); ?>"
				...
				?>
 				...
 				<td>
 					<a class="btn btn-sm btn-primary"
 						href="pembayaran.php?act=bayar&id=<?php echo $data->id; ?>">Bayar</a>
 					<!-- <a class="btn btn-sm btn-danger"
 						href="pesanan.php?act=delete&id=<?php echo $data->id ?>">Batalkan</a> -->
 				</td>
 			</tr>
 			<?php } ?>
 		</tbody>
