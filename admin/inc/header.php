...
<body>
  ...
  $qpesanan = mysqli_query($db, "select*from pesanan where `read`='0'") or die (mysqli_error());
  $totalUnRead = mysqli_num_rows($qpesanan);
  // TOTAL PEMBAYARAN YANG BELUM DI VERIFIKASI
  $qPembayaran = mysqli_query($db, "select*from pembayaran where `status`='pending'") or die (mysqli_error());
