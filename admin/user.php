<?php 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			extract($_POST); 
			$password = md5($password);
			$q = mysqli_query($db, "update user SET nama='$nama',email='$email',telephone='$telephone',alamat='$alamat',password='$password',status='$status' where id=$_GET[id]") or die(mysqli_error());
			if($q){ alert("Success"); redir("user.php"); }
		}
	}
