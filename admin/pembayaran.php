...
<div class="container">
	...
	<div class="row">
		<div class="col-md-12">
			...
			if(!empty($_GET)){
				if ($_GET['act'] == "info") {
					$qInfo = mysqli_query($db, "Select * from info_pembayaran limit 1") or die (mysqli_error());
					$dInfo = mysqli_fetch_object($qInfo);
					if (!empty($_POST)) {
						extract($_POST);
						$query = mysqli_query($db, "Update info_pembayaran set info='$info' where id='1'") or die (mysqli_error());
						if ($query) {
			...
			<?php
				...
				if ($_GET['act'] == "verified") {
					...
					if ($updatePembayaran) {
						$qPembayaran = mysqli_query($db, "Select * from pembayaran where id_pesanan='$id_pesanan' and status='verified'") or die (mysqli_error());
						$totalPembayaran = 0;
						...
						$q3 = mysqli_query($db, "select*from pesanan where id='$id_pesanan'") or die (mysqli_error());
						$pesanan = mysqli_fetch_object($q3);

						$totalBayar += $pesanan->ongkir;
						if ($totalBayar <= $totalPembayaran) {
							$updatePesanan = mysqli_query($db, "update pesanan set status='lunas' where id='$id_pesanan'") or die (mysqli_error());
							if ($updatePesanan) {
