<?php 
	...
	$aa = mysqli_query($db, "update pesanan SET `read`='1' where id=$_GET[id]") or die(mysqli_error());
	// if(!empty($_GET)){
		// if($_GET['act'] == 'delete'){
	...
	// if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		// if(!empty($_POST)){
			...
			// $q = mysqli_query($db, "update pesanan SET tanggal_pesan='$tanggal_pesan',tanggal_digunakan='$tanggal_digunakan',user_id='$user_id',alamat='$alamat',telephone='$telephone' where id=$_GET[id]") or die(mysqli_error());
			// if($q){ alert("Success"); redir("pesanan.php"); }
		// }
	// }
...
<div class="container">
	<?php
			...
			$ongkir = $data->ongkir;
			$kota = $data->kota;
			$dataPembayaran = mysqli_query($db, "select * from pembayaran where id_pesanan='$data->id' and status='verified'") or die (mysqli_error());
			$totalPembayaran = 0;
