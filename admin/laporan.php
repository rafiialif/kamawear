...
<div class="container" id="printableArea">
	...
	<?php
		if (@$_GET['act'] != "cetak") {
			?>
	<input type="button" class="btn btn-primary" onclick="printDiv('printableArea')" value="Cetak" />
	<?php
		}
	?>
	<div class="col-md-12">
		...
		<table class="table table-striped" border="1">
			...
			<tbody>
				<?php
					...
					$totalOngkir = 0;
					$no = 0;
					$q = mysqli_query($db, "Select pesanan.* from pesanan order by id desc") or die (mysqli_error());
					while ($data = mysqli_fetch_object($q)) {
						$totalHarga = 0;
						$no++;
						$q2 = mysqli_query($db, "Select detail_pesanan.*, produk.harga from detail_pesanan INNER JOIN produk ON detail_pesanan.produk_id = produk.id where pesanan_id = '$data->id'") or die (mysqli_error());
